...
function Imh () {
  ...
  this.ligne_commande = {
    ...
    'ajouter': function (spt) {
      if (this.liste.length > this.indice + 1) { this.liste.length = this.indice + 1 }
      this.indice += 1
  ...
  this.picking = {
    ...
    'toggle': function () {
      this.toggle_proxy()
      if (!this.value) { pan_commande.halos.value = true }
    },
    'command': {
...
function actualise_legende (donnees) {
  ...
  for (i = 0; i < donnees.length; i++) {
    if (typeof (donnees[i].text) === 'undefined') { break } else if (donnees[i].text === '|') { txt += '<br>' } else {
...
function activer_panneau (panneau) {
  ...
  for (i = 0; i < panneaux.length; i++) {
    if (panneau === panneaux[i]) { pan_commande.panneau.show(panneaux[i]) } else { pan_commande.panneau.hide(panneaux[i]) }
  }
}
...
function actualise_cell () {
  ...
  spt = 'display '
  for (x in cellules) {
    if (typeof (cellules[x]) === 'string') { spt += 'cell=' + cellules[x] + ' or ' }
  }
