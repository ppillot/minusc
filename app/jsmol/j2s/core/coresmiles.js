...
Clazz_declarePackage ("JS");
Clazz_load (["J.api.SmilesMatcherInterface"], "JS.SmilesMatcher", ["JU.AU", "$.BS", "$.PT", "JS.InvalidSmilesException", "$.SmilesAtom", "$.SmilesBond", "$.SmilesGenerator", "$.SmilesParser", "JU.BSUtil", "$.Logger", "$.Node", "$.Point3fi"], function () {
...
var array = this.findPriv (pattern, search, (isSmarts ? 131072 : 1114112) | (firstMatchOnly ? 4194304 : 0) | 12288);
...
var debug = JU.Logger.debugging;
JU.Logger.debugging = false;
var s = this.getSmiles (atoms, atomCount, JU.BSUtil.newBitSet2 (0, atomCount), null, 1 | 4 | (points == null ? 2 : 65536));
JU.Logger.debugging = debug;
return s;
}, "~A,~N,~A");
...
if (!JS.SmilesSearch.checkStereochemistryAll (sAtom.not != this.invertStereochemistry, atom0, chiralClass, order, jn[0], jn[1], jn[2], jn[3], null, null, this.v)) return false;
continue;
case 4:
case 8:
case 5:
...
if (!JS.SmilesSearch.checkStereochemistryAll (sAtom.not != this.invertStereochemistry, atom0, chiralClass, order, atom1, atom2, atom3, atom4, atom5, atom6, this.v)) return false;
continue;
}
}
...
this.min =  Clazz_newFloatArray (4, 0);
this.max =  Clazz_newFloatArray (4, 0);
this.points =  new Array (4);
});
Clazz_makeConstructor (c$, 
function (search, index, type, isNot) {
this.search = search;
this.type = Math.min (4, Math.max (type, 2));
this.index = index;
this.isNot = isNot;
}, "JS.SmilesSearch,~N,~N,~B");
Clazz_defineMethod (c$, "addRange", 
function (min, max) {
if (this.n >= 4) return;
this.min[this.n] = Math.min (min, max);
this.max[this.n] = Math.max (min, max);
this.n++;
}, "~N,~N");
Clazz_overrideMethod (c$, "toString", 
function () {
var s = "(." + "__dat".charAt (this.type) + this.index + ":" + JU.PT.toJSON (null, this.min) + "," + JU.PT.toJSON (null, this.max) + ") for";
...
for (var i = 0; i < this.n; i++) if (d >= this.min[i] && d <= this.max[i]) return !this.isNot;
...
Clazz_declarePackage ("JS");
Clazz_load (["java.util.Hashtable"], "JS.SmilesParser", ["java.lang.Character", "$.Float", "JU.Lst", "$.PT", "$.SB", "JS.InvalidSmilesException", "$.SmilesAtom", "$.SmilesBond", "$.SmilesMeasure", "$.SmilesSearch", "JU.Elements", "$.Logger"], function () {
c$ = Clazz_decorateAsClass (function () {
this.isSmarts = false;
...
if (strFlags.indexOf ("INVERTSTEREO") >= 0) this.flags |= 4;
}
...
if (molecule.isTopology && (atom.hasSubpattern || atom.iNested != 0 || atom.isBioAtom || atom.elementNumber != -2 || atom.nAtomsOr > 0 || atom.nPrimitives > 0)) molecule.isTopology = false;
atom.setBondArray ();
...
if (tokens.length % 2 == 1 || tokens.length > 8) break;
var vals =  Clazz_newFloatArray (tokens.length, 0);
var i = tokens.length;
for (; --i >= 0; ) if (Float.isNaN (vals[i] = JU.PT.fVal (tokens[i]))) break;

if (i >= 0) break;
m =  new JS.SmilesMeasure (molecule, index, type, isNot);
molecule.measures.addLast (m);
for (i = 0; i < vals.length; i += 2) m.addRange (vals[i], vals[i + 1]);
